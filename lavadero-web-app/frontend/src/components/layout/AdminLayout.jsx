...
const AdminLayout = () => {
  ...
  return (
    <div className="min-h-screen bg-linear-to-br from-slate-50 to-slate-100 flex">
      <aside className={`bg-linear-to-b from-slate-900 via-slate-800 to-slate-900 text-white transition-all duration-500 ${sidebarOpen ? 'w-72' : 'w-20'} flex flex-col shadow-2xl border-r border-slate-700`}>
        {/* Header con Logo */}
        <div className="p-4 border-b border-slate-700 bg-linear-to-r from-slate-800 to-slate-700">
          <div className="flex items-center justify-between mb-3">
            {sidebarOpen && (
              <div className="flex items-center gap-3 flex-1">
                <div className="relative">
                  <div className="absolute inset-0 bg-linear-to-r from-amber-500 to-orange-500 rounded-lg opacity-75 blur"></div>
                  <img src="/assets/icons/logo_crm.png" alt="Logo" className="relative w-12 h-12 object-contain" />
                </div>
                <div>
                  <h1 className="text-lg font-bold leading-tight bg-linear-to-r from-amber-400 to-orange-400 bg-clip-text text-transparent">Lavadero</h1>
            ...
            {!sidebarOpen && (
              <div className="relative mx-auto">
                <div className="absolute inset-0 bg-linear-to-r from-amber-500 to-orange-500 rounded-lg opacity-50 blur"></div>
                <img src="/assets/icons/logo_crm.png" alt="Logo" className="relative w-10 h-10 object-contain" />
              </div>
            )}
            <button onClick={() => setSidebarOpen(!sidebarOpen)} className="p-2 hover:bg-slate-700 rounded-lg transition-all duration-300 shrink-0 hover:scale-110">
        ...
        <div className="p-4 border-t border-slate-700 bg-linear-to-r from-slate-800 to-slate-700">
          {sidebarOpen ? (
            <div className="mb-3">
            ...
            <div className="w-10 h-10 bg-linear-to-br from-amber-500 to-orange-500 rounded-full flex items-center justify-center mx-auto mb-3 shadow-lg">
              <span className="text-lg font-bold text-white">{(user?.username || 'A')[0].toUpperCase()}</span>
            </div>
          )}
          <button 
            onClick={handleLogout} 
            className={`w-full flex items-center gap-2 p-3 bg-linear-to-r from-red-600 to-red-700 hover:from-red-700 hover:to-red-800 rounded-xl transition-all duration-300 group hover:shadow-lg hover:shadow-red-700/50 hover:scale-105 ${!sidebarOpen ? 'justify-center' : ''}`}
