...
public class Modelo303DTO {
    ...
    public void calcularResultado() {
        ...
        if (cuotaDevengada21 != null)
            totalCuotaDevengada = totalCuotaDevengada.add(cuotaDevengada21);
        if (cuotaDevengada10 != null)
            totalCuotaDevengada = totalCuotaDevengada.add(cuotaDevengada10);
        if (cuotaDevengada4 != null)
            totalCuotaDevengada = totalCuotaDevengada.add(cuotaDevengada4);

        // Total deducible
        this.totalCuotaDeducible = BigDecimal.ZERO;
        if (cuotaDeducibleInteriores != null)
            totalCuotaDeducible = totalCuotaDeducible.add(cuotaDeducibleInteriores);
        if (cuotaDeducibleInversion != null)
            totalCuotaDeducible = totalCuotaDeducible.add(cuotaDeducibleInversion);
        if (cuotaDeducibleImportaciones != null)
            totalCuotaDeducible = totalCuotaDeducible.add(cuotaDeducibleImportaciones);

        // Diferencia
        this.diferencia = totalCuotaDevengada.subtract(totalCuotaDeducible);

        // Resultado (considerando compensaciones)
        if (cuotasCompensar == null)
